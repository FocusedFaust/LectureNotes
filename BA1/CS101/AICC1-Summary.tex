...
\begin{figure}[htp]
\subfloat[$\forall x\forall y$]{
\begin{tikzpicture}
\foreach \Point in {(0,0), (1,0), (0,1), (1,1), (0,2), (1,2), (0,3), (1,3)}{
    \node at \Point {\textbullet};
}
\foreach \Point in {(0,0), (0,1), (0,2), (0,3)}{
	\draw[-] \Point -- (1,0);
	\draw[-] \Point -- (1,1);
	\draw[-] \Point -- (1,2);
	\draw[-] \Point -- (1,3);
...
\end{tikzpicture}}
\hfill
\subfloat[$\exists x\forall y$]{
\begin{tikzpicture}
\foreach \Point in {(0,0), (1,0), (0,1), (1,1), (0,2), (1,2), (0,3), (1,3)}{
    \node at \Point {\textbullet};
}
\draw[-] (0,0) -- (1,1);
\foreach \Point in {(1,0), (1,1), (1,2), (1,3)}{
    \draw[-] (0,2) -- \Point;
}
\end{tikzpicture}}
\hfill
\subfloat[$\exists x\exists y$]{
\begin{tikzpicture}
\foreach \Point in {(0,0), (1,0), (0,1), (1,1), (0,2), (1,2), (0,3), (1,3)}{
    \node at \Point {\textbullet};
}
...
\par 
We can see here some visual representation of nested quantifiers. The segments represent the pairs $(x,y)$ for which an arbitrary $P(x)$ would be true according to the quantifiers. This is not something to learn by heart but rather to understand. Then, it will come naturally. 
...
\label{section:substraction_rule}
...
\section{Extra tips and tricks}
I don't have much to add to make counting easier but I remember one thing that a teacher said about both counting and probability. In essence, if the question is about the probability of events $A$ \textbf{and} $B$, you should multiply the respective probabilities. However, if the question is about the probability of events $A$ \textbf{or} $B$, you should add their respective probabilities. \par 
\textbf{Warning:} Be careful about that, it is not automatically true. Therefore you should always check your understanding of the problem and your calculations. (I cannot be held responsible if this technique doesn't work.) I have added a few examples for you to better understand what I mean.
\paragraph{Examples} 
\begin{itemize}
\item Let the probability of me drinking milk be $p(milk)$ and the probability of me eating a dal\footnote{A delicious Indian meal made with lentils (hence the name, dal means lentil in Sanskrit).} be $p(dal)$. The probability of me having \textbf{milk and a dal} for lunch would be $p(milk)\cdot p(dal)$.
\item Let the probability of a cow having three spots be $p(3)$ and the probability of it having five spots be $p(5)$. The probability of a cow having \textbf{3 or 5} spots would be $p(3)+p(5)$.
\end{itemize} 
Notice how when I use "\textbf{or}", the events do not overlap (they are incompatible), otherwise, we would have to use the Substraction Rule (see \ref{section:substraction_rule}). I could replace "or" by "either ... or" and the meaning would not have been changed.
